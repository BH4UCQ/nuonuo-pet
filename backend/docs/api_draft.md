...
- `GET /api/device/{device_id}/display/profile`
- `GET /api/device/{device_id}/template/selection`
- `GET /api/device/{device_id}/display/profile/batch`
...
- `device/capability/summary` 可以给后端/工具侧使用，`device/{device_id}/display/profile` 则更偏向设备本身的显示消费。
- `template/selection` 可以被 firmware 或前端直接消费，里面带有推荐 species/theme、display_mode、display_hint、layout_name、scene_hint、preview_asset 和槽位统计。
- `preview/sample` 会返回推荐的 `display_mode`、`display_hint`、`layout_name` 和 `scene_hint`。
- `preview` 接口返回 palette、ui_slots、layers、notes，并尽量保持模板安全。
...
### GET /api/device/{device_id}/summary
- 聚合返回 device detail + health + display_profile + sync_minimal，给前端/固件一次取齐。


### GET /api/device/{device_id}/summary/minimal
- 给 ESP32 / 轻量前端的压缩版设备总览。
- 优先返回 `binding_state`、`connection_state`、`health_level`、`summary_line`、`action_hint`、`display_mode`、`layout_name`、`scene_hint`。
